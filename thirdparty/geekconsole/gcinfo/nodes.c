...
info_get_node (char *filename, char *nodename)
{
  ...
  if (!node
      /* XXX: test here about ather special nodes */
  ...
  while ((nodestart = find_node_separator (&binding)) != -1)
    {
      ...
      entry = malloc (sizeof (TAG));
      entry->nodename = malloc (1 + (end - start));
    ...
    SEARCH_BINDING *indirect_binding, SEARCH_BINDING *tags_binding)
{
    ...
    while (line < end)
      {
        ...
        subfile = malloc (sizeof (SUBFILE));
        subfile->filename = malloc (colon);
  ...
  if (!subfiles)
  ...
  else
    {
      ...
      {
        char *containing_dir = strdup (file_buffer->fullpath);
        ...
        file_buffer->subfiles = malloc ((1 + i) * sizeof (char *));
...
info_node_of_file_buffer_tags (FILE_BUFFER *file_buffer, char *nodename)
{
  ...
  // first search nodename in tag, if not found try ignore case
  for (i = 0; (tag = file_buffer->tags[i]); i++)
    if (strcmp (nodename, tag->nodename) == 0)
      goto prtag;

  for (i = 0; (tag = file_buffer->tags[i]); i++)
    if (mbscasecmp (nodename, tag->nodename) == 0)
      {
      prtag:
        {
...
{
  FILE_BUFFER *file_buffer = malloc (sizeof (FILE_BUFFER));
          ...
          if (offset != -1)
            {
              ...
              if (((unsigned int) offset == strlen (node->nodename)) &&
                  (mbsncasecmp (node->nodename, nodedef, offset) == 0))
                {
