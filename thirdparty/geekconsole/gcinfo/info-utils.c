...
/* Return the node addressed by LABEL in NODE (usually one of "Prev:",
   ...
   the information. */
void
info_parse_label_line (char *label, char *nodeline)
{
  register int i;
  ...
  while ((position = search_forward (label, &tmp_search)) != -1)
    {
      ...
      entry = malloc (sizeof (REFERENCE));
      entry->filename = NULL;
      entry->nodename = NULL;
      entry->label = malloc (offset);
...
info_concatenate_references (REFERENCE **ref1, REFERENCE **ref2)
{
  ...
  result = malloc ((1 + size) * sizeof (REFERENCE *));
...
{
  REFERENCE *dest = malloc (sizeof (REFERENCE));
  dest->label = src->label ? strdup (src->label) : NULL;
  dest->filename = src->filename ? strdup (src->filename) : NULL;
  dest->nodename = src->nodename ? strdup (src->nodename) : NULL;
  ...
  if (!string)
  ...
  else
    {
      if (len >= *string_size_p)
        *string_p = realloc (*string_p, (*string_size_p = 1 + len));
