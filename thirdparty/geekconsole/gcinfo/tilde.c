  ...
  while (1)
    {
      ...
      if ((result_index + start + 1) > result_size)
        result = realloc (result, 1 + (result_size += (start + 20)));
      ...
      if ((result_index + len + 1) > result_size)
        result = realloc (result, 1 + (result_size += (len + 20)));
...
{
  char *dirname = filename ? strdup (filename) : NULL;
      ...
      if (!dirname[1] || IS_SLASH (dirname[1]))
        {
          ...
          temp_name = malloc (1 + strlen (&dirname[1])
      ...
      else
        {
          ...
          if (!(user_entry = (struct passwd *) getpwnam (username)))
            ...
            {
              temp_name = malloc (1 + strlen (user_entry->pw_dir)
                                   + strlen (&dirname[i])); 
...
/* XXX: tilde for windows? this lib will compiled not with cygwin
   Original info from texinfo compiled not by MSC.
*/
#endif /* _MSC_VER */
  return dirname;
}
