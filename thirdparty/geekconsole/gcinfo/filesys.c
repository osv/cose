  ...
  while ((temp_dirname = extract_colon_unit (path, &dirname_index)))
    {
      ...
      temp = malloc (30 + strlen (temp_dirname) + strlen (filename));
...
{
  char *containing_dir = strdup (fname);
  ...
  if (names_and_files_index + 2 > names_and_files_slots)
    {
      ...
      names_and_files = realloc (names_and_files, alloc_size);
    }

  new = malloc (sizeof (FILENAME_LIST));
  new->filename = strdup (filename);
  new->expansion = expansion ? strdup (expansion) : NULL;
...
maybe_initialize_infopath (void)
{
  if (!infopath_size)
    {
      infopath = (char *)
        malloc (infopath_size = (1 + strlen (DEFAULT_INFOPATH)));
...
info_add_path (char *path, int where)
{
  ...
  if (!infopath)
    {
      infopath = malloc (infopath_size = 200 + strlen (path));
      infopath[0] = '\0';
    }
  ...
  if (len + 2 >= infopath_size)
    infopath = realloc (infopath, (infopath_size += (2 * len) + 2));
  ...
  if (compressed_filename_p (pathname))
  ...
  else
    {
	 ...
	 want to waste storage.  */
      if (*filesize < st_size)
	contents = realloc (contents, 1 + *filesize);
      contents[*filesize] = '\0';
...
filesys_read_compressed (char *pathname, long int *filesize)
{
  ...
  *filesize = filesys_error_number = 0;
  ...
  command = malloc (15 + strlen (pathname) + strlen (decompressor));
  ...
  if (stream)
    {
      ...
      offset = size = 0;
      chunk = malloc (FILESYS_PIPE_BUFFER_SIZE);
...
filesys_error_string (char *filename, int error_num)
{
  ...
  if (len >= errmsg_buf_size)
    errmsg_buf = realloc (errmsg_buf, (errmsg_buf_size = 2 + len));
