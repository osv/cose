...
void CelestiaCore::draw()
{
    ...
    else
    {
             ...
             iter != views.end(); iter++)
        {
            ...
            {
                glScissor((GLint) (view->x * width)+x,
                          (GLint) (view->y * height)+y,
                          (GLsizei) (view->width * width),
                          (GLsizei) (view->height * height));
                glViewport((GLint) (view->x * width)+x,
                           (GLint) (view->y * height)+y,
                           (GLsizei) (view->width * width),
                           (GLsizei) (view->height * height));
...
void CelestiaCore::resize(GLsizei x, GLsizei y, GLsizei w, GLsizei h)
{
    if (h == 0)
	h = 1;
    this->x = x;
...
void CelestiaCore::renderOverlay()
{
    ...
    // we may want render flash at any time
    // so use flashFrame() to flash current frame
    if (currentTime < flashFrameStart + 0.5 &&
	(views.size() > 1
	 || (views.size() == 1 && getHudDetail() > 0)))
    {
	glLineWidth(8.0f);
	glColor4f(0.5f, 0.5f, 1.0f,
		  (float) (1.0 - (currentTime - flashFrameStart) / 0.5));
