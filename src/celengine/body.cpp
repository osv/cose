...
Body::~Body()
{
    ...
    if (altSurfaces)
    {
        AltSurfaceTable::iterator iter;
        for (iter = altSurfaces->begin();
             iter != altSurfaces->end(); iter++)
        {
            delete (Surface*)iter->second;
        }
        altSurfaces->clear();
	delete altSurfaces;
    }
    if (locations)
    {
        for_each(locations->begin(), locations->end(), deleteFunc<Location*>());;
        delete locations;
    }
...
//alloc body , surf, loc
// timelinephase
// planets = new PlanetarySystem(star);solarsys.cpp:1283
// 0x816c469 is in SolarSystem (solarsys.cpp:1284).
// 1283	    planets = new PlanetarySystem(star);
// 1284	    frameTree = new FrameTree(star);

// 0x819a13a is in Universe::createSolarSystem(Star*) const (universe.cpp:155).
// 154	
// 155	    solarSystem = new SolarSystem(star);

// 0x81af4fb is in CreateFixedRotationModel (parseobject.cpp:881).
// 880	
// 881	    return new ConstantOrientation(q);

// 0x81b4e0c is in CreateOrbit(Selection const&, AssociativeArray*, std::string const&, bool) (parseobject.cpp:756).
// 755	
